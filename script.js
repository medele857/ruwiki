...
function triggerFoxyScreamer() {
  ...
  var vid = document.getElementById('foxyVideo');
...
// ══════════════════════════════════════
(function() {
  var SCALE_KEY = 'siteScale';
  var FONT_KEY  = 'siteFont';

  var FONTS = [
    { id: 'default', label: 'По умолчанию', css: "'MC', 'Courier New', monospace" },
    { id: 'readable', label: 'Читаемый',    css: "'Trebuchet MS', sans-serif" },
    { id: 'mc',       label: 'Minecraft Rus', css: "'MinecraftRus', 'MC', monospace" },
    { id: 'mc2',      label: 'MC Classic',   css: "'Courier New', monospace" },
  ];

  var MIN_SCALE = 80, MAX_SCALE = 140, STEP = 10;

  // Load saved settings
  var savedScale = parseInt(localStorage.getItem(SCALE_KEY) || '100', 10);
  var savedFont  = localStorage.getItem(FONT_KEY) || 'default';
  ...
  function applyFont(id) {
    var f = FONTS.find(function(x){ return x.id === id; }) || FONTS[0];
    // Override --font CSS variable
  ...
  if (scaleMinus) {
    scaleMinus.addEventListener('click', function() {
      savedScale = Math.max(MIN_SCALE, savedScale - STEP);
      applyScale(savedScale);
      localStorage.setItem(SCALE_KEY, savedScale);
      if (scaleVal) scaleVal.textContent = savedScale + '%';
    });
  }
  if (scalePlus) {
    scalePlus.addEventListener('click', function() {
      savedScale = Math.min(MAX_SCALE, savedScale + STEP);
      applyScale(savedScale);
      localStorage.setItem(SCALE_KEY, savedScale);
      if (scaleVal) scaleVal.textContent = savedScale + '%';
    });
  ...
  document.querySelectorAll('.font-opt').forEach(function(btn) {
    if (btn.dataset.fontId === savedFont) btn.classList.add('active');
    btn.addEventListener('click', function() {
      savedFont = btn.dataset.fontId;
      applyFont(savedFont);
      localStorage.setItem(FONT_KEY, savedFont);
